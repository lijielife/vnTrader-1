...
from ctaStrategy.ctaEngine2 import CtaEngine2
...
class MainEngine(object):
    ...
    #----------------------------------------------------------------------
    def isGatewayConnected(self, gatewayName):
        """Gateway是否已经连接"""
        if gatewayName in self.gatewayDict:
            gateway = self.gatewayDict[gatewayName]
    ...
    def sendOrder(self, orderReq, gatewayName):
        ...
        if gatewayName in self.gatewayDict:
            ...
            self.writeLog(text.GATEWAY_NOT_EXIST.format(gateway=gatewayName))        
    #----------------------------------------------------------------------
    def sendParkedOrder(self, orderReq, gatewayName):
        """对特定接口发单"""
        # 如果风控检查失败则不发单
        ...
        if gatewayName in self.gatewayDict:
            gateway = self.gatewayDict[gatewayName]
            return gateway.sendParkedOrder(orderReq)
        else:
            self.writeLog(text.GATEWAY_NOT_EXIST.format(gateway=gatewayName))   

    #----------------------------------------------------------------------
    def qryOrder(self, vtOrderID, gatewayName):
        if gatewayName in self.gatewayDict:
            gateway = self.gatewayDict[gatewayName]
            gateway.qryOrder(vtOrderID)
        else:
            self.writeLog(text.GATEWAY_NOT_EXIST.format(gateway=gatewayName))
        ...
        if not self.dbClient:
            ...
            try:
                ...
                if logging:
                    ...
                    self.eventEngine.register(EVENT_CTA_LOG, self.dbLogging)
                    self.eventEngine.register(EVENT_ERROR, self.dbLoggingError)
                    
            except ConnectionFailure:
                self.writeLog(text.DATABASE_CONNECTING_FAILED)
    
    #----------------------------------------------------------------------
    def dbInsert(self, dbName, collectionName, d, ignore_field=''):
        """向MongoDB中插入数据，d是具体数据"""
        
        dc = copy.copy(d)
        if ignore_field in dc:
            del dc[ignore_field]
        if self.dbClient:
            db = self.dbClient[dbName]
            collection = db[collectionName]
            if '_id' in dc:
                collection.update_one({'_id':d['_id']}, {'$set':dc}, upsert=True)
            else:
    ...
    #----------------------------------------------------------------------
    def dbQuery(self, dbName, collectionName, d, where=''):
        ...
        if self.dbClient:
            ...
            collection = db[collectionName]
            try:
                cursor = collection.find(d).where(where)
    ...
    def printLog(self, event):
        ...
        print '%s\t%s\t%s' %(log.logTime, log.gatewayName, log.logContent)

    #----------------------------------------------------------------------
    def printErrorLog(self, event):
        """向控制台输出Error日志"""
        error = event.dict_['data']
        print 'Err: %s\t%s\t%s\t%r' %(error.errorTime, error.gatewayName, error.errorMsg, error.errorID)
    ...
    def getGateway(self, vtSymbol):
        """由vtSymbol得到对应的gateway"""
        contract = self.getContract(vtSymbol)
        if contract:
            gatewayName = contract.gatewayName
            return self.gatewayDict.get(gatewayName)
        else:
            return None
