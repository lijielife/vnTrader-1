...
if __name__ == '__main__':
    df1 = pd.read_csv('results/result1.csv', parse_dates=['datetime'], index_col='datetime',usecols=['datetime', 'bar1_close', 'bar1_close_pf','wf_result'])
    df1 = df1.rename(columns={'bar1_close':'close', 'bar1_close_pf':'pf_close','wf_result':'wfr_1min'})
    ...
    df = pd.concat([df1,dfpos], axis=1).fillna(method='ffill')
    df1 = df5 = df60 = None
    fig1 = plt.subplot(311)
    fig1.plot(list(df['close']))
    fig1.plot(list(df['pf_close']))
    fig2 = plt.subplot(312)
    fig2.plot(list(df['wfr_1min']))
    #fig2.plot((df['wfr_5min']))
    #fig2.plot((df['wfr_60min']))
