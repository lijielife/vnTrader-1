...
class ShzdGateway(VtGateway):
    ...
    def sendOrder(self, orderReq):
        """发单"""
        id = self.api.sendOrder(orderReq)
        return id
    #----------------------------------------------------------------------
    def qryOrder(self, vtOrderID):
        self.api.qryOrder(vtOrderID)
    #----------------------------------------------------------------------
...
class ShzdGatewayApi(ShzdApi):
    ...
    def initCallbacks(self):
        ...
        self.tradeCallbacks['O2'] = self.onBalance
    #----------------------------------------------------------------------
    ...
    def onOrder(self, data):
        ...
        if orderID:
            ...
            if order.status != STATUS_CANCELLED:
                self.gateway.onOrder(copy(order))
    #----------------------------------------------------------------------
    def onBalance(self, data):
        if not data['404'] or data['404'] == '00000':
            balance = VtBalanceData()
            balance.gatewayName = self.gatewayName
            balance.oldCanUse = float(data['217'])
            balance.todayAmount = float(data['204'])
            balance.expiredProfit = float(data['205'])
            balance.frozenDeposit = float(data['210'])
            balance.fee = float(data['211'])
            balance.deposit = float(data['212'])
            balance.keepDeposit = float(data['213'])
            balance.inMoney = float(data['214'])
            balance.outMoney = float(data['215'])
            balance.unexpiredProfit = float(data['216'])
            balance.unaccountProfit = float(data['224'])
            balance.royalty = float(data['412'])
            #deposit unfrozen
            self.gateway.onBalance(copy(balance))
    #----------------------------------------------------------------------
    ...
    def connect(self, userId, userPwd, 
        ...
        time.sleep(1)
        # 处理累积的订阅请求
        for req in self.subscribe_list:
            self.subscribe(req)
        self.subscribe_list = []
    ...
    def subscribe(self, subscribeReq):
        ...
        if not self.connected:
            self.subscribe_list.append(subscribeReq)
            return
        req = {}
        req['msgtype'] = 'MA'
        req['11'] = self.accountNo
        req['201'] = '+'
        req['307'] = ','.join([exchangeMap[str(subscribeReq.exchange)], str(subscribeReq.symbol)])
    ...
    def sendOrder(self, orderReq):
        ...
        req['401'] = priceTypeMap.get(priceType, '')
        ...
        vtOrderID = '.'.join([self.gatewayName, req['305']])
        return vtOrderID
    ...
    def qryOrder(self, vtOrderID=''):
        """委托查询"""
        print 'qryOrder in SHZD'
        req = {}
        req['msgtype'] = 'ORS'
        req['12'] = self.userId
        req['11'] = self.accountNo
        if vtOrderID:
            localID = '.'.join(vtOrderID.split('.')[1:])
            req['201'] = 'S'
            req['300'] = localID
        self.shzdSendInfoToTrade(req)         
