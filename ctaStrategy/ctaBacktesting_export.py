...
class BacktestingEngine(object):
    ...
    def runBacktesting(self):
        ...
        with codecs.open(self.dir + 'log.txt', 'a', 'utf-8') as f:
    ...
    # ----------------------------------------------------------------------
    def sendOrder(self, vtSymbol, orderType, price, volume, strategy=None):
        """发单"""
    ...
    def writeCtaLog(self, content):
        ...
        dt_str = ''
        if self.dt:
            dt_str = self.dt.strftime('%Y-%m-%d %H:%M:%S.%f')
        log = '[' + dt_str + '] ' + content + '\n'
        print content
    ...
    def calculateBacktestingResult(self):
            ...
            if trade.direction == DIRECTION_LONG:
                ...
                # 当前多头交易为平空
                    ...
                    while True:
                        ...
                        # 清算开平仓交易
                        closedVolume = min(
                            exitTrade.volume, entryTrade.volume)
                        result = TradingResult(entryTrade.price, entryTrade.dt,
                                                exitTrade.price, exitTrade.dt,
                                                -closedVolume, self.rate, self.slippage, self.size, entryTrade.vtOrderID, exitTrade.vtOrderID)
                        resultList.append(result)

                        posList.extend([-1, 0])
                        tradeTimeList.extend(
                            [result.entryDt, result.exitDt])
        ...
        pd.DataFrame(prettyResultList).to_csv(self.dir + 'trades.csv')
    ...
    def showBacktestingResult(self):
        ...
        self.output(u'平均每笔盈利：\t%s' % formatNumber(
            d['capital']/d['totalResult']))
        self.output(u'平均每笔滑点：\t%s' % formatNumber(
            d['totalSlippage']/d['totalResult']))
        self.output(u'平均每笔佣金：\t%s' % formatNumber(
            d['totalCommission']/d['totalResult']))
        ...
        if d['posList'][-1] == 0:
            del d['posList'][-1]
        tradeTimeIndex = [item.strftime("%m/%d %H:%M:%S")
                          for item in d['tradeTimeList']]
...
class TradingResult(object):
    ...
    def __init__(self, entryPrice, entryDt, exitPrice,
                 exitDt, volume, rate, slippage, size, entryOrderId='', exitOrderId=''):
        """Constructor"""
        ...
        self.turnover = (self.entryPrice+self.exitPrice) * \
            size*abs(volume)   # 成交金额
        self.commission = self.turnover*rate                                # 手续费成本
        self.slippage = slippage*2*size * \
            abs(volume)                         # 滑点成本
        self.pnl = ((self.exitPrice - self.entryPrice) * volume * size
                    - self.commission - self.slippage)                      # 净盈亏
        self.entryOrderId = entryOrderId
        self.exitOrderId = exitOrderId
