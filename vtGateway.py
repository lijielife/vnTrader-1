...
class VtGateway(object):
    ...
    def onBalance(self, balance):
        """下单资金返回推送"""
        event1 = Event(type_=EVENT_BALANCE)
        event1.dict_['data'] = balance
        self.eventEngine.put(event1)
    #----------------------------------------------------------------------
...
########################################################################
class VtBalanceData(VtBaseData):
    """资金返还数据类"""

    #----------------------------------------------------------------------
    def __init__(self):
        """Constructor"""
        super(VtBalanceData, self).__init__()
        
        # 代码编号相关
        self.gatewayName = EMPTY_STRING         # 接口名称

        # 资金相关
        self.oldCanUse = EMPTY_UNICODE          # 昨可用
        self.todayAmount = EMPTY_UNICODE        # 今结存
        self.expiredProfit = EMPTY_FLOAT        # 平仓盈亏
        self.frozenDeposit = EMPTY_INT          # 冻结资金
        self.fee = EMPTY_FLOAT                  # 手续费
        self.deposit = EMPTY_FLOAT              # 保证金
        self.keepDeposit = EMPTY_FLOAT          # 维持保证金
        self.inMoney = EMPTY_FLOAT              # 入金
        self.outMoney = EMPTY_FLOAT             # 出金
        self.unexpiredProfit = EMPTY_FLOAT      # 未到期平盈
        self.unaccountProfit = EMPTY_FLOAT      # 未结平盈
        self.royalty = EMPTY_FLOAT              # 期权利金
