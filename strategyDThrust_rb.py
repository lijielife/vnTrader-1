...
__BACKTESTING__ = False
BACKTESTING_SYMBOL = 'rb9999'
TICK_SIZE = 1
STATE_FILE = 'temp/state.DThrust'
if __BACKTESTING__:
...
class State1(State):
    ...
    def onEnterState(self):
        ...
        # self.strategy.writeStrategyLog('trend_index = %f' %self.strategy.get_trend_index())
...
class State1cd(State):
    ...
    def inState(self):
        ...
        if self.need_trailing_stoploss():
            ...
            self.new_state(State5)
            return
        if self.strategy.lastTick.lastPrice > self.long_trigger:
        ...
        elif self.strategy.lastTick.lastPrice < self.short_trigger:
            #print self.strategy.lastPrice, self.strategy.lastTick.lastPrice, self.long_trigger
    ...
    def onEvent(self, event_type):
        ...
        #     # 1cd ---> 1
        #     self.strategy.writeStrategyLog('1cd ---> 8: MARKET CLOSE')
    ...
    def onEnterState(self):
        ...
        if pos > 0:
            ...
            if self.direction == DIRECTION_LONG:
                ...
                # =============================================================================================
                self.strategy.writeStrategyLog('vtSymbol=%s last=%.1f working_vtOrderId=%s direction=%s'
                                               % (self.strategy.vtSymbol, self.strategy.lastTick.lastPrice,
                                                  self.working_vtOrderId, self.direction))
                # =============================================================================================
                # 2 ---> 2cd
...
class State3(State):
    ...
    def onEnterState(self):
        pass
        #self.strategy.writeStrategyLog('Enter S3')
...
########################################################################
class DThrust_rbStrategy(CtaTemplate2):
    
    className = 'DThrust_rbStrategy'
    author = u''
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(DThrust_rbStrategy, self).__init__(ctaEngine, setting)
        #self.lastPrice = 0.0
        self.barSeries_1min = []
    ...
    def onTrade(self, trade):
        ...
        self.fsm.inState()
        #self.writeStrategyLog(str(self.pos_cost_dict))
    ...
    def ismarketopen(self):
        ...
        tmpTime = self.get_now_time().time()
...
def backtesting():
    ...
    # 在引擎中创建策略对象
    engine.initStrategy(DThrust_rbStrategy, {})
